...
export class Barbarian implements PresetProvider{
	...
	calculate(type: string, level: number, accuracyProvider: AccuracyProvider, accuracyMode: string, resources: BarbarianResources, options?: BarbarianOptions): { damage: number; accuracy: number; } {
		...
		if (type == 'frenzy' && level >= 3) {
			...
			hitDamage += 10;
			critDamage += 10;
			total = (1-resources.recklessPercent)*Util.getDamageWithCrits(this.attacks(level), hitDamage, critDamage, adjustedFlat.hit, adjustedFlat.crit)+resources.recklessPercent*Util.getDamageWithCrits(this.attacks(level), hitDamage, critDamage, adjustedAdvantage.hit, adjustedAdvantage.crit);
		} else {
